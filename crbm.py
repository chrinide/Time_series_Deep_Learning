...
from motion import load_data_self, load_data_label


class CRBM(object):
    """Conditional Restricted Boltzmann Machine (CRBM)  """
    def __init__(self, input=None, input_history=None, n_visible=31,
    ...
    def get_cost_updates(self, lr=0.1, k=1, momentum=0.9):
        """
        ...
        mom = T.cast(momentum, dtype=theano.config.floatX)
        ...
        for gparam, param in zip(gparams, self.params):
            ...
            if param == self.A:
                ...
                upd = mom * param_update - 0.01 *lr * gparam
                updates[param_update] = upd
                ...
                param_update = theano.shared(param.get_value(), broadcastable=param.broadcastable)
                upd = mom * param_update - lr * gparam
                updates[param_update] = upd
                updates[param] = (param + upd)
...
def train_crbm(learning_rate=0.01, training_epochs=1000,
    ...
    cost, updates = crbm.get_cost_updates(lr=learning_rate, k=6, momentum=momentum)
...
if __name__ == '__main__':
    ...
    data_idx = np.array([100, 200, 300, 400])
    ...
    for i in xrange(len(generated_series)):
        ...
        plt.plot(bd[start - crbm.delay:start + 100 - crbm.delay, 0],
                 label='true', linestyle=':')
        plt.plot(generated_series[i, :100, 0], label='predicted',
                 linestyle='-')
